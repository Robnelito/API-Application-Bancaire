...
const getClientSolde = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      error: "Une erreur est survenue lors de la récupération du solde du client",
    });
  }
};
...
const effectuerTransfert = async (req, res) => {
  ...
  try {
    const soldeSource = await transfertModel.getClientSolde(numero_compte_source);
    const soldeDestination = await transfertModel.getClientSolde(numero_compte_destination);

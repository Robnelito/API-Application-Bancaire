...
const getClients = (req, res) => {
    pool.query('SELECT * FROM client ORDER BY numero_compte ASC', (error, results) => {
        if (error) {
            throw error
        }
...
const getClient = (req, res) => {
    ...
    pool.query('SELECT * FROM client WHERE numero_compte = $1', [numero_compte], (error, results) => {
        if (error) {
            throw error
        }
        res.status(200).json(results.rows)
...
const createClient = (req, res) => {
    ...
    pool.query('INSERT INTO client (nom, solde) VALUES ($1, $2) RETURNING *', [nom, solde], (error, results) => {
        if (error) {
            throw error
        }
        res.status(201).send(`User added with ID: ${results.rows[0].numero_compte}`)
    })
}
